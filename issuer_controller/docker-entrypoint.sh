...
export APP_MODULE=${APP_MODULE:-${APP_NAME}.${APP_NAME}:${APP_NAME}}
...
CMD="$@"
if [ -z "$CMD" ]; then
  CMD="gunicorn -k ${WORKER_CLASS} --reload --worker-connections ${WORKER_CONNECTIONS} --bind ${HOST_IP}:${HOST_PORT} ${APP_MODULE}"
fi
